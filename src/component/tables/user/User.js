...
const User = () => {
  ...
  const userTable = [
    ...
    },
    {
      ...
      body: "profileImage",
      Cell: ({ row }) => (
        <div className="userProfile" style={{ height: "50px", width: "50px", overflow: "hidden" }}>
    ...
    { Header: "Gender", body: "gender", sorting: { type: "client" } },
    {
      ...
      body: "isOnline",
      Cell: ({ row }) => (
        <span className={`${row?.isOnline === true ? "bg-success-light text-success" : "bg-danger-light text-danger"} text-light p4 fs-10`}>{row?.isOnline ? "Online" : "Offline"}</span>
      ),
      sorting: { type: "client" }
    },
    {
      ...
      body: "isBlock",
      Cell: ({ row }) => (
        <span className={`${row?.isBlock === false ? "bg-success-light text-success" : "bg-danger-light text-danger"} text-light p4 fs-10`}>{row?.isBlock ? "Block" : "Unblock"}</span>
      ),
      sorting: { type: "client" }
    },
      ...
      <div className='bg-light p15'>
        <div className="row justify-content-between align-items-center">
          <div className="col-2 m0">
            <Button className={`bg-second text-light`} text={`ADD`} bIcon={`ri-add-line`} onClick={() => {
