...
const Attributes = () => {
  ...
  const attributesTable = [
    ...
    { Header: "Attributes Name", body: "attrName", sorting: { type: "client" },  width: "200px" },
    {
      Header: "Details",
      body: "details",
      width: "800px",
        ...
        <span className='attrDetails' style={{minWidth:"400px"}}>
          {
            row?.details?.map((res)=>{
              return(
                <div>
                  {row.attrName == "Color" && <span style={{backgroundColor:`${res}`}} className='colorBall'></span>}
                  <span>{res}</span>
                </div>
              )
            })
      ...
      Cell: ({ row }) => (
        <span>
          <button
            ...
            onClick={() => dispatch(openDialog({ type: "attributes", data: row }))}
          >Edit</button>
        </span>
      ),
    }
